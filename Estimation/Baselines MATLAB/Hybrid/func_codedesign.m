...
for l=1:S
    ...
    for k=1:2^l
        ...
        w_hyb_temp = func_improve_hie_filters(g,N,A_BS,wd_temp);
        ...
        if control_plot ==1
            if l == l_plot
                ...
                polarplot(theta,rho);
                hold on;
                rho = abs(A_BS'*w_hyb_temp);
                polarplot(theta,rho);
                hold on;
                legend('fully-digital filter','hybrid filter');
            end
        end            
        w_D{l}{k} = w_hyb_temp;
    end
...
save('output_func_code_design.mat','w_D','A_BS','A_BS_pinv','theta','A_BS_OS','theta_OS');
