...
val_size_order = 23 #Validation_set_size = val_size_order*delta_inv
...
with tf.name_scope("channel_sensing"):
    ...
    mu = mean_true_alpha*tf.ones(shape=[tf.shape(phi_input)[0],delta_inv_OS],dtype=tf.complex64)
    SIG2 = (2*std_per_dim_alpha**2)*tf.ones(shape=[tf.shape(phi_input)[0],delta_inv_OS],dtype=tf.float32)  
    ...
    for t in range(tau):
        ...
        y_complex_tile = tf.tile(y_complex,(1,delta_inv_OS))
        'BS estimates alpha by Kalman'
        G = tf.complex(tf.sqrt(lay['P']),0.0)* w_her_complex@A_BS_OS
        ...
        posteriors_OS = (f*posteriors_OS)/(tf.reduce_sum(f*posteriors_OS,axis=1,keepdims=True)+0.00000001) #added to avoid numerical errors
        posteriors = tf.reduce_sum(tf.reshape(posteriors_OS,[-1,delta_inv,OS_rate]),axis=2)
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        if epoch%100==99: #Every 10 iterations it checks if the validation performace is improved, then saves parameters
...
sio.savemat('data_DNN_unknown_alpha_Kalman_EST_hyb_norm.mat',dict(performance= performance,\
