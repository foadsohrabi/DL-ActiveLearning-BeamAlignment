...
tf.disable_v2_behavior() # This is important to use tf.v1 modules with tf.v2
import numpy as np
...
with tf.name_scope("channel_sensing"):
    ...
    for t in range(tau):
        ...
        mean_complex = tf.complex(tf.sqrt(lay['P']),0.0)*alpha_input*(w_her_complex@A_BS)
        unnorm_post = tf.exp(-tf.pow(tf.abs(y_complex - mean_complex),2))
        posteriors_temp = tf.multiply(unnorm_post,posteriors)
...
sio.savemat('data_DNN_known_alpha.mat',dict(performance= performance,\
