...
batch_size_order = 26 #Mini_batch_size = batch_size_order*delta_inv
val_size_order = 600 #Validation_set_size = val_size_order*delta_inv
...
with tf.name_scope("channel_sensing"):
    ...
    for t in range(tau):
        ...
        Y_tile = tf.tile(tf.reshape(Y,[-1,t+1,1]),(1,1,delta_inv))  
        
        'BS estimates alpha by MMSE'
        c = tf.complex(tf.sqrt(lay['P']),0.0)*\
            tf.reshape(tf.reshape(W_her, [-1, N]) @ A_BS, [-1, t+1, delta_inv])
        ...
        alpha_hat_extended = tf.tile(tf.reshape(alpha_hat,[-1,1,delta_inv]),(1,t+1,1))
        ...
        posteriors = tf.divide(posteriors_temp,sum_posteriors_temp)
...
with tf.Session() as sess:
    ...
    best_loss, pp, alpha_hat_val = sess.run([loss,posterior_dict,alpha_hat], feed_dict=feed_dict_val)
    print(best_loss)
...
sio.savemat('data_DNN_unknown_alpha_MMSE_updatePIs.mat',dict(performance= performance,\
